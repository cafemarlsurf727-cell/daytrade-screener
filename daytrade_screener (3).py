        ...
        steps:
          ...
          - name: レポートHTMLをコミット
            run: |
              ...
              git add docs/index.html logs/signal_log.csv
...
# ============================================================================
class SignalLogger:
    """シグナルの発生記録と、事後の勝敗・損益（R倍数）を管理するクラス。"""

    _COLUMNS = [
        "signal_date", "ticker", "name", "strategy_code", "strategy_label", "side",
        "entry_price", "stop_loss", "max_shares", "risk_amount",
        "ad_ratio", "status",
        "actual_open", "actual_high", "actual_low", "actual_close",
        "stopped_out", "exit_price", "gap_pct",
        "pnl_per_share", "pnl_total", "r_multiple", "resolved_at",
    ]

    def __init__(self, log_path: str = "logs/signal_log.csv") -> None:
    ...
    def load(self) -> pd.DataFrame:
        if self.log_path.exists():
            try:
                ...
                logger.warning("シグナルログの読み込みに失敗しました: %s", exc)
        return pd.DataFrame(columns=self._COLUMNS)

    def save(self, df: pd.DataFrame) -> None:
        self.log_path.parent.mkdir(parents=True, exist_ok=True)
        df.to_csv(self.log_path, index=False)
        logger.info("シグナルログを保存しました: %s (%d件)", self.log_path, len(df))

    def append_new(
        self, df: pd.DataFrame, signals: list[dict], breadth: dict[str, float], signal_date: str
    ) -> pd.DataFrame:
        """本日検出したシグナルを status=pending として追記する。"""
        if not signals:
            return df
        new_rows = []
        for s in signals:
            ...
            new_rows.append(row)
        appended = pd.concat([df, pd.DataFrame(new_rows)], ignore_index=True)
        logger.info("シグナルログに新規 %d件を記録しました（pending）。", len(new_rows))
        return appended

    def resolve(self, df: pd.DataFrame, data_dict: dict[str, pd.DataFrame]) -> pd.DataFrame:
        """
        pending状態の行のうち、その後の取得データに signal_date の実績
        （始値・高値・安値・終値）が含まれているものを答え合わせする。
        """
        if df.empty:
            return df

        # CSV往復後にNaN列がfloat64等と推測され、bool/文字列の代入で
        # LossySetitemError（pandas 2.x）になるのを防ぐため、
        # 書き込み対象の列をobject dtypeに揃えておく。
        write_cols = [
            "actual_open", "actual_high", "actual_low", "actual_close",
            "stopped_out", "exit_price", "gap_pct",
            "pnl_per_share", "pnl_total", "r_multiple", "resolved_at", "status",
        ]
        ...
        for idx in df[pending_mask].index:
            ...
            ohlc = data_dict[ticker]
            # インデックスを日付文字列(YYYY-MM-DD)で照合する
            match_idx = [d for d in ohlc.index if str(d)[:10] == signal_date[:10]]
            if not match_idx:
                continue  # まだその日の確定データが来ていない

            bar = ohlc.loc[match_idx[0]]
            ...
            side = row["side"]
            max_shares = float(row["max_shares"])

            if side == "buy":
                stopped_out = actual_low <= stop_loss
                exit_price = stop_loss if stopped_out else actual_close
                pnl_per_share = exit_price - entry_price
            else:  # sell（空売り）
                stopped_out = actual_high >= stop_loss
                exit_price = stop_loss if stopped_out else actual_close
                pnl_per_share = entry_price - exit_price

            risk_per_share = abs(entry_price - stop_loss)
            r_multiple = pnl_per_share / risk_per_share if risk_per_share > 0 else pd.NA
            gap_pct = (actual_open - entry_price) / entry_price * 100.0 if entry_price else pd.NA

            df.loc[idx, "actual_open"] = actual_open
            df.loc[idx, "actual_high"] = actual_high
            df.loc[idx, "actual_low"] = actual_low
            df.loc[idx, "actual_close"] = actual_close
            df.loc[idx, "stopped_out"] = stopped_out
            df.loc[idx, "exit_price"] = exit_price
            df.loc[idx, "gap_pct"] = gap_pct
            df.loc[idx, "pnl_per_share"] = pnl_per_share
            df.loc[idx, "pnl_total"] = pnl_per_share * max_shares
            df.loc[idx, "r_multiple"] = r_multiple
            df.loc[idx, "resolved_at"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            df.loc[idx, "status"] = "resolved"
            resolved_count += 1

        if resolved_count:
            logger.info("シグナルログを %d件 答え合わせしました。", resolved_count)
        return df

    @staticmethod
    def summarize(df: pd.DataFrame) -> dict:
        """戦略コードごとの勝率・平均R倍数・サンプル数を集計する。"""
        ...
        resolved["win"] = resolved["r_multiple"] > 0

        def _agg(sub: pd.DataFrame) -> dict:
            n = len(sub)
            win_rate = float(sub["win"].mean() * 100.0) if n else 0.0
            avg_r = float(sub["r_multiple"].mean()) if n else 0.0
            return {"n": n, "win_rate": win_rate, "avg_r": avg_r}

        overall = _agg(resolved)
        by_strategy = {
            code: _agg(sub) for code, sub in resolved.groupby("strategy_code")
        }
...
class Notifier:
    ...
    def format_message(signals: list[dict], breadth: dict[str, float]) -> str:
        ...
        lines = [
            ...
            "※すべて当日大引け(15:30)で全決済すること。持ち越し厳禁。",
            "",
        ]
        ...
        <section class="breadth-panel">
          <div class="breadth-row">
            ...
            <span class="breadth-value">
              勝率 {overall['win_rate']:.0f}% / 平均R {overall['avg_r']:+.2f} / n={overall['n']}
            </span>
          </div>
...
class TradingSystem:
    ...
    def __init__(
        ...
        pages_url: Optional[str] = None,
        log_path: Optional[str] = "logs/signal_log.csv",
    ) -> None:
        self.tickers = tickers
        self.names = names or {}
        self.capital = capital
        ...
        self.signal_logger = SignalLogger(log_path=log_path) if log_path else None
    ...
    def run(self, dry_run: bool = False) -> list[dict]:
        ...
        # 5) シグナルログ：過去分の答え合わせ → 本日分の記録 → 保存
        stats = None
        if self.signal_logger:
            log_df = self.signal_logger.load()
            log_df = self.signal_logger.resolve(log_df, data_dict)
            today_str = datetime.now().strftime("%Y-%m-%d")
            log_df = self.signal_logger.append_new(log_df, actionable, breadth, today_str)
            self.signal_logger.save(log_df)
            stats = self.signal_logger.summarize(log_df)

        # 6) レポートHTML生成（GitHub Pages公開用）
        if self.report_path:
        ...
        # 7) 通知（pages_url指定時はLINEに詳細ページへのリンクを添える）
        notifier = Notifier(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--log-path",
        type=str,
        default="logs/signal_log.csv",
        help="シグナルの記録・答え合わせ用CSVの出力先。空文字を指定すると記録をスキップする。",
    )
